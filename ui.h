...
void render_cc_ctrl(EEPROMStorage<uint16_t> smin, EEPROMStorage<uint16_t> smax, EEPROMStorage<uint16_t> tmin, EEPROMStorage<uint16_t> tmax, WiFiClient client){
  String tpl = F(
    "<form method='post' action='/cc'><fieldset>"
    "<div class='float-right'><input type='submit' value='Update'></div><h3>MIDI CC</h3>"
    "<label for='cc_smin'>Source MIN CC</label>"
    "<input type='number' id='cc_smin' name='cc_smin' value='{{cc_smin}}' min=0 max=1000>"
    "<label for='cc_smax'>Source MAX CC</label>"
    "<input type='number' id='cc_smax' name='cc_smax' value='{{cc_smax}}' min=0 max=1000>"
    "<label for='cc_tmin'>Target MIN CC</label>"
    "<input type='number' id='cc_tmin' name='cc_tmin' value='{{cc_tmin}}' min=0 max=127>"
    "<label for='cc_tmax'>Target MAX CC</label>"
    "<input type='number' id='cc_tmax' name='cc_tmax' value='{{cc_tmax}}' min=0 max=127>"
    "</fieldset></form><hr>"
  );
...
void cc_post_handler(String url, String name, String value){
  if(url.endsWith("cc")){
    if(name.compareTo("cc_smin") == 0) cc_smin.set(atoi(value.c_str()));
    
    if(name.compareTo("cc_smax") == 0) cc_smax.set(atoi(value.c_str()));
    
    if(name.compareTo("cc_tmin") == 0) cc_tmin.set(atoi(value.c_str()));
    
    if(name.compareTo("cc_tmax") == 0) cc_tmax.set(atoi(value.c_str()));
  }
}

void render_note_ctrl(int id, uint16_t trig_min, uint16_t trig_max, uint16_t note, WiFiClient client){
  String tpl = F(
...
void cc_note_handler(String url, String name, String value){
  if(url.startsWith("/note")){
    ...
    if(name.compareTo("trig_min") == 0) note.min = atoi(value.c_str());
    if(name.compareTo("trig_max") == 0) note.max = atoi(value.c_str());
...
post_form_handler post_handlers[num_post_handlers] = { &_debug_post, 
                                                  &midi_post_handler,
                                                  &cc_post_handler,
                                                  &cc_note_handler,
                                              };  
...
void post_req_cb(ArduinoHttpServer::HttpResource resource, String body){
  ...
  for (tok = strtok_r(arg_buff, arg_del, &saved); tok; tok = strtok_r(NULL, arg_del, &saved))
  {
